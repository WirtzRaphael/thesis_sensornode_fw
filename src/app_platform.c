...
void PL_InitWatchdogReportTable(void) {
  ...
  #endif
  #if PL_CONFIG_USE_GUI_ENERGY_DASHBOARD
  McuWatchdog_InitReportEntry(McuWatchdog_REPORT_ID_TASK_EMONITOR, "eMon", 1000, 70, 120);
  #endif
  #if PL_CONFIG_USE_ROAD
  McuWatchdog_InitReportEntry(McuWatchdog_REPORT_ID_TASK_ROAD, "Road", 1000, 70, 120);
  #endif
}
#endif
...
void pico_usb_get_unique_board_id_string(char *id_out, uint len) {
  ...
  McuUtility_strcpy(id_out, len, "PicoVerkehrshaus");
#endif
}
...
void PL_Init(void) {
  ...
  McuGPIO_Init();
  McuLED_Init();
  //McuTimeDate_Init(); /* initialization will be done in task context */
#if PL_CONFIG_USE_RTT
  McuRTT_Init();
#endif
#if configUSE_SEGGER_SYSTEM_VIEWER_HOOKS
